...
class WebServer:
	def __init__(self, host='0.0.0.0', captivePortalIP='', port=80, max_conn=5):
		...
		routeHandlers = [
			...
			( "/rl_run", "GET", lambda cli, *arg: execRL(cli.GetRequestQueryString(True), cli._addr[0])),
			( "/rl_exec", "POST", lambda cli, *arg: execRL(cli.ReadRequestContent(), cli._addr[0])),
...
def run():
	...
	gc.collect()
	SetTimer('syncNTP', 36000, True, syncNTP)
	g.DEBUG_dpin(0)
